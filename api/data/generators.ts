...
export const agvs: AgvItem[] = Array.from({ length: AGV_COUNT }, (_, i) => {
  const id = `AGV-${String(i + 1).padStart(3, '0')}`;
  const statuses: AgvItem['status'][] = ['working', 'charging', 'idle', 'offline', 'abnormal'];
  const status = i < 3 ? 'working' : i < 8 ? 'charging' : i < 12 ? 'idle' : i === 12 ? 'abnormal' : 'offline';
  return {
    id,
    name: `机器人 ${i + 1}`,
    status,
...
export const devices: DeviceItem[] = [
  ...
  ...Array.from({ length: FLOOR_COUNT * 2 }, (_, i) => {
  ...
  ...Array.from({ length: 12 }, (_, i) => ({
    ...
    position: { x: (Math.random() - 0.5) * 50, y: 7, z: (Math.random() - 0.5) * 30, floor: 1 },
    floor: 1,
  ...
  for (let r = 0; r < SHELF_ROWS; r++) {
    for (let c = 0; c < SHELF_COLS; c++) {
      ...
      for (let l = 0; l < LOCATIONS_PER_SHELF; l++) {
        ...
        if (status === 'occupied') {
          const containerCode = `${String.fromCharCode(65 + Math.floor(Math.random() * 26))}${String.fromCharCode(65 + Math.floor(Math.random() * 26))}${String(Math.floor(Math.random() * 100)).padStart(2, '0')}${String(Math.floor(Math.random() * 1000)).padStart(3, '0')}.${Math.floor(Math.random() * 10)}`;
          boxes.push({
            containerCode,
...
export const tasks: TaskItem[] = Array.from({ length: 24 }, (_, i) => {
  const statuses: TaskItem['status'][] = ['pending', 'running', 'completed', 'failed'];
  const status = i < 8 ? 'running' : i < 12 ? 'pending' : i < 22 ? 'completed' : 'failed';
  return {
    ...
    target: `S-${((i + 2) % 5) + 1}-${String(((i + 3) % 6) + 1).padStart(2, '0')}-${String(((i + 5) % 10) + 1).padStart(2, '0')}`,
...
export function getOverview(): WarehouseOverview {
  const robotStatus = {
    total: agvs.length,
  ...
  return { robotStatus, locationStatus, powerDistribution, taskStats, chargingStatus };
}
...
export function generateAlarm(): AlarmItem | null {
